    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-sm animate-in fade-in duration-200 p-4">
      <div className="w-full max-w-5xl bg-zinc-950 border border-zinc-800 rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 border-b border-white/5 flex justify-between items-start bg-[url('/grid-pattern.svg')]">
          ...
          <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-full text-zinc-400 hover:text-white transition-colors"><X className="w-8 h-8" /></button>
        </div>
          ...
          {match.status === 'live' && showSensitiveInfo && (
              <div className="mb-8 p-6 bg-fuchsia-900/5 border border-fuchsia-500/20 rounded-lg animate-in slide-in-from-bottom-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="bg-black/40 p-4 rounded border border-white/5 flex justify-between items-center group cursor-pointer hover:border-fuchsia-500/50 transition-colors" onClick={() => handleCopy(match.server_ip, 'IP')}>
                          <div>
                      ...
                      <div className="bg-black/40 p-4 rounded border border-white/5 flex justify-between items-center group cursor-pointer hover:border-fuchsia-500/50 transition-colors" onClick={() => handleCopy(match.server_pass, 'Password')}>
                          <div>
                ...
                {match.status === 'scheduled' && (
                    <div className="bg-fuchsia-900/10 border border-fuchsia-500/20 p-6 rounded-lg text-center">
                        ...
                        <div className="flex justify-center gap-4">
                            <RestrictedButton action={PERM_CAPABILITIES.ACT_AS_CAPTAIN} context={match} className="px-6 py-2 bg-fuchsia-600 hover:bg-fuchsia-500 text-white font-bold uppercase rounded text-sm transition-all shadow-lg shadow-fuchsia-900/20" onClick={() => alert("Ready Check: Coming in v1.1")}>
                                Ready Check
                            </RestrictedButton>
                        </div>
                    </div>
