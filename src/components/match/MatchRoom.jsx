...
export const MatchRoom = ({ matchId }) => {
  ...
  const isParticipant = session?.user && (match?.team1_id === session?.identity?.team_id || match?.team2_id === session?.identity?.team_id);
  ...
  return (
    <div className="min-h-screen bg-bg text-white p-6 md:p-12">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        {/* TEAM 1 */}
        <TeamCard team={match.team1} score={match.team1_score} isWinner={match.winner_id === match.team1_id} />

        {/* CENTER CONTROL */}
        <div className="space-y-6">
            
            {/* SERVER INFO (Secure Display) */}
            {match.status === 'live' && isParticipant && match.server_ip && (
                <div className="bg-emerald-950/20 border border-emerald-500/30 rounded-lg p-4 animate-in fade-in">
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between items-center bg-black/40 p-2 rounded border border-emerald-500/10 cursor-pointer hover:bg-black/60" onClick={() => copyToClipboard(`connect ${match.server_ip}`)}>
        ...
        {/* TEAM 2 */}
        <TeamCard team={match.team2} score={match.team2_score} isWinner={match.winner_id === match.team2_id} />
