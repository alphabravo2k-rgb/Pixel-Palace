...
import { Shield, Swords, Clock, AlertCircle, LogOut, CheckCircle, Trophy, User, RefreshCw } from 'lucide-react';
...
export const PlayerDashboard = () => {
  ...
  // --- THE FIX: TRIPLE-SOURCE TEAM ID ---
  // We check 3 places to ensure PIN-based logins get their Team ID
  const getTeamId = () => {
    // 1. Check User Metadata (Email-based login)
    if (session?.user?.user_metadata?.team_id) return session.user.user_metadata.team_id;
    // 2. Check Session Object (PIN-based login)
    if (session?.team_id || session?.teamId) return session.team_id || session.teamId;
    // 3. Check LocalStorage (Backup for refresh)
  ...
  const displayName = session?.user?.user_metadata?.display_name || session?.teamName || 'Operator';
  ...
  const fetchData = async () => {
    ...
    try {
      // Find the most urgent match (including scheduled matches so they see it early)
      const { data: match, error } = await supabase
        .from('matches')
        .select(`*, team1:team1_id(name, logo_url), team2:team2_id(name, logo_url)`)
        .or(`team1_id.eq.${teamId},team2_id.eq.${teamId}`)
        .in('status', ['scheduled', 'veto', 'live', 'disputed']) // Added 'disputed'
  ...
  useEffect(() => {
    fetchData();
    // Auto-refresh dashboard every 30 seconds to catch bracket updates
  ...
  const handleLogout = async () => {
    await logout();
    localStorage.removeItem('pixel_captain_session'); // Clear the backup
    navigate('/login');
  };
    ...
    <div className="min-h-screen bg-bg text-white p-6 selection:bg-brand/30">
      ...
      <div className="max-w-5xl mx-auto flex justify-between items-center border-b border-white/5 pb-6 mb-8">
          ...
          <div className="flex gap-2">
            <button onClick={fetchData} className="p-3 bg-zinc-900 hover:bg-zinc-800 rounded-full border border-zinc-800 text-zinc-500 hover:text-white transition-all">
                <RefreshCw size={20} />
            </button>
          ...
          <div className={cn(
            ...
            {activeMatch ? (
               <div className="p-8 relative z-10">
                  <div className="flex justify-between items-start mb-8">
                     <div>
                        ...
                        <p className="text-zinc-500 text-xs font-mono mt-1 tracking-wider">
                           MATCH #{activeMatch.match_no || activeMatch.match_position} • STATUS: <span className={cn("font-bold", activeMatch.status === 'live' ? "text-red-500" : "text-white")}>{activeMatch.status.toUpperCase()}</span>
                        </p>
                     </div>
                  ...
                  <Button 
                      variant="brand" 
                      className="w-full py-6 text-lg tracking-widest font-black italic"
               ...
               <div className="p-12 text-center">
                  ...
                  {!teamId && (
                      <div className="mt-6 bg-red-900/20 border border-red-700/30 p-4 rounded text-red-500 text-xs font-mono animate-pulse">
                          ⚠️ NO UNIT ASSIGNED. CONTACT ADMIN IMMEDIATELY.
                      </div>
                  )}
               </div>
            )}
