...
// --- ASSETS: REGION FLAGS & ICONS ---
const getRegionFlag = (regionCode) => {
  if (!regionCode) return null;
  const code = regionCode.toUpperCase();
  // Map common codes to standard ISO 2-letter codes for FlagCDN
  const maps = { 
    'PAK': 'pk', 'PK': 'pk', 
    'IND': 'in', 'IN': 'in', 
    'IRN': 'ir', 'IR': 'ir', 
  ...
  if (isoCode) {
    return (
      <img 
        ...
        alt={code} 
        title={`Region: ${code}`}
        className="h-3 w-4 object-cover rounded-[2px] shadow-sm opacity-80 hover:opacity-100 transition-opacity" 
      />
    );
  }
  // Fallback for "ME" (Middle East) or generic text
  return (
    <span className="text-[9px] font-black bg-zinc-800 text-zinc-400 px-1 rounded border border-zinc-700 cursor-help" title={`Region: ${code}`}>
      {code.substring(0, 2)}
    </span>
  );
};
...
// --- SMART SOCIAL BUTTON COMPONENT ---
const SocialLink = ({ href, handle, id, type }) => {
  ...
  const isClickable = finalUrl || handle;
  ...
  const Component = finalUrl ? 'a' : 'button';
  const props = finalUrl ? { href: finalUrl, target: '_blank', rel: 'noreferrer' } : { onClick: handleAction };

  return (
    <Component 
        {...props}
        className={`p-1.5 rounded border border-transparent transition-all ${colors} group relative`} 
        onClick={(e) => { e.stopPropagation(); if (!finalUrl) handleAction(e); }}
        title={finalUrl ? `Open ${type}` : `Copy: ${handle}`}
    >
      {copied ? <Check className="w-3 h-3 text-green-500 animate-in zoom-in" /> : <Icon className="w-3 h-3" />}
    </Component>
  );
};
           ...
           return (
             <div key={m.id} className={`flex items-center justify-between px-3 py-1.5 rounded hover:bg-white/5 transition-colors ${isSub ? 'opacity-60' : ''}`}>
               ...
               <div className="flex gap-0.5 shrink-0">
                 ...
                 <SocialLink href={null} handle={m.discord} id={m.discord_id} type="Discord"/>
               </div>
             </div>
           );
        })}
...
const EditTeamModal = ({ team, onClose, onRefresh, tournamentId }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      let teamId = team?.id;
      const teamData = { tournament_id: tournamentId, ...meta };
      ...
      for (const m of members) {
        const identityPayload = {
            display_name: m.username, steam_url: m.steam_url || null,
            faceit_url: m.faceit_url || null, discord_handle: m.discord || null,
