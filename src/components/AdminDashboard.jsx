...
const AdminDashboard = () => {
  ...
  const goToBracket = () => navigate('/', { state: { skipLogin: true } });
  const handleLogin = (e) => { e.preventDefault(); login(pin); };

  const handleCreate = (e) => {
    e.preventDefault();
    createAdmin(pin, { name: newAdmin.name, discord: newAdmin.discordHandle, discordUser: newAdmin.discordUser, faceitUser: newAdmin.faceitUser, faceitUrl: newAdmin.faceitUrl });
  };

  const handleChangePin = async (e) => {
    e.preventDefault();
    setSuccessMsg('');
    const success = await changeMyPin(changePinData.oldPin, changePinData.newPin, { discordHandle: adminProfile.discord_handle, faceitUser: adminProfile.faceit_username, faceitUrl: adminProfile.faceit_url }, changePinData.securityToken);
    ...
    return (
      <div className="min-h-screen bg-[#050505] flex items-center justify-center p-4 font-['Rajdhani']">
         <div className="bg-[#0a0a0c] p-10 rounded-xl border border-zinc-800 shadow-2xl w-full max-w-md relative overflow-hidden">
            ...
            <h2 className="mb-8 text-2xl font-black text-white tracking-widest text-center font-['Teko'] uppercase">Admin Access</h2>
  ...
  return (
    <div className="min-h-screen bg-[#060709] p-4 md:p-12 font-['Rajdhani'] bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-fixed">
      <div className="w-full max-w-6xl mx-auto bg-[#0b0c0f]/95 border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden backdrop-blur-md">
        ...
        <header className="bg-black/40 border-b border-zinc-800 p-6 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-black text-white italic tracking-tighter font-['Teko'] uppercase">Command <span className="text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-500 to-purple-500">Console</span></h1>
