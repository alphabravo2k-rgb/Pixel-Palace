...
import { Search, Users, Crown, AlertTriangle, MessageCircle, Shield, Pin, ExternalLink } from 'lucide-react';
...
const SocialButton = ({ href, type }) => {
  ...
  if (!href) {
    return (
      <div className="p-1.5 opacity-20 cursor-not-allowed group/social" title={`${label} not provided`}>
  ...
  return (
    <a href={href} target="_blank" rel="noreferrer" className={`p-1 text-zinc-500 transition-all duration-200 ${colors} hover:scale-110 active:scale-95`} title={`Open ${label} Profile`} onClick={(e) => e.stopPropagation()}>
...
const PlayerRow = ({ player }) => {
  ...
  // Cyber-Theme Colors (Fuchsia/Purple vs Blue/Yellow)
  const tagColor = isCap 
    ...
    : isSub 
      ...
      : 'bg-fuchsia-900/20 text-fuchsia-400 border border-fuchsia-500/20';

  return (
    <div className={`relative group w-full h-12 border-b border-white/5 last:border-0 flex items-center overflow-hidden will-change-transform ${isSub ? 'bg-black/40' : 'bg-[#0a0a0c]'}`}>
        ...
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded bg-zinc-900 border border-white/10 flex items-center justify-center overflow-hidden relative">
            {player.avatar ? (
               ...
               <span className="text-[10px] font-bold opacity-30 tracking-tighter text-zinc-500">{displayName.substring(0,2).toUpperCase()}</span>
            )}
          </div>
          <div className="flex items-center gap-2">
            {(isCap || isSub) && <Pin className={`w-2.5 h-2.5 rotate-45 ${isCap ? 'text-blue-400 fill-blue-400/20' : 'text-yellow-500'}`} />}
      ...
      <div className="absolute inset-0 z-20 flex items-center justify-between px-4 bg-[#0a0a0c] transform translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out">
        ...
        <span className="text-[10px] font-medium text-zinc-100 uppercase tracking-tighter truncate max-w-[120px] px-4 font-['Rajdhani']">
           {displayName}
        </span>
      ...
      {/* Hover Neon Accent */}
      <div className={`absolute left-0 top-0 h-full w-[2px] opacity-0 group-hover:opacity-100 transition-opacity ${isCap ? 'bg-blue-400 shadow-[0_0_8px_#3b82f6]' : 'bg-fuchsia-500 shadow-[0_0_8px_#d946ef]'}`} />
    </div>
  );
};

const GhostRow = () => (
  <div className="relative w-full h-12 border-b border-white/5 last:border-0 flex items-center bg-white/[0.02] overflow-hidden">
    ...
    <div className="px-4 flex items-center gap-3 opacity-10 grayscale">
      ...
      <span className="text-[9px] font-mono uppercase tracking-[0.4em] italic text-zinc-500">Tactical Reserve</span>
    </div>
  </div>
);
...
const TeamCard = ({ team }) => {
  ...
  return (
    <div className="group relative bg-[#0b0c0f]/80 border border-white/10 hover:border-fuchsia-500/50 transition-all duration-300 flex flex-col h-full overflow-hidden shadow-2xl backdrop-blur-sm"
      ...
      <div className="p-4 bg-gradient-to-r from-[#15191f] to-[#0b0c0f] border-b border-white/10 flex items-center justify-between relative z-10">
        <div className="flex items-center gap-3">
           ...
           <div className="flex flex-col">
               <h3 className="text-white font-black text-lg truncate max-w-[140px] uppercase italic tracking-tighter font-['Teko']">
                 {team.name}
               </h3>
               <div className="flex items-center gap-2">
                 <span className="text-[10px] text-fuchsia-500/80 font-mono tracking-widest">SEED #{team.seed_number}</span>
                 {team.is_synced && <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_8px_#10b981]" title="Live Sync Active" />}
               </div>
           </div>
        </div>
        {flagUrl && (
          <img src={flagUrl} alt={team.region} title={team.region} className="w-8 h-5 rounded-sm shadow-sm opacity-60 hover:opacity-100 transition-opacity border border-white/10" />
        )}
      </div>

      <div className="flex flex-col flex-grow bg-[#0b0c0f]/50 relative z-10">
        {sortedPlayers.map((p, idx) => <PlayerRow key={p.id || idx} player={p} />)}
...
const TeamRoster = () => {
  ...
  if (error) return (
     <div className="p-12 text-center text-red-500 font-mono text-xs uppercase flex flex-col items-center gap-3">
        <AlertTriangle className="w-8 h-8" /><span>Sync Failure: {error}</span>
     </div>
  );
