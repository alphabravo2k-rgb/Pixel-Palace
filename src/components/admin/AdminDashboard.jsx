...
import { useSession } from '../../auth/useSession'; // 🟢 FIXED: Up 2 levels (admin -> components -> auth)
import { 
  ShieldAlert, 
  Trophy, 
  ScrollText, 
...
// 🟢 FIXED: Import from parent components folder
...
export const AdminDashboard = () => {
  ...
  const handleLogout = async () => {
    await logout();
    navigate('/login'); // Unified Login Route
  };
      ...
      <main className="max-w-7xl mx-auto p-6">
        ...
        {activeTab === 'USERS' && (
          <div className="p-12 text-center border border-dashed border-zinc-800 rounded text-zinc-500 font-mono">
            User Management Module: Restricted Access
          </div>
        )}
      </main>
    </div>
