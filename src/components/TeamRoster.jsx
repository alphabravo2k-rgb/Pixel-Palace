...
// Map 3-letter codes (common in esports) to ISO-2 codes for flagcdn
const COUNTRY_MAP = {
...
const getFlagUrl = (region) => {
  ...
  const code = COUNTRY_MAP[cleanRegion] || 'un'; // Default to 'un' (United Nations/World) if unknown
  return `https://flagcdn.com/w40/${code}.png`;
};

// Safe Faceit Name Extractor
...
const SocialButton = ({ href, type }) => {
  if (!href) {
    ...
    return <div className="p-1.5 opacity-20"><Icon className="w-4 h-4" /></div>;
  }
...
const PlayerRow = ({ player }) => {
  ...
  // Avatar logic: DB > Default
  const avatarUrl = player.avatar || null;

  return (
    <div className={`relative group w-full h-12 border-b border-zinc-800/50 last:border-0 flex items-center overflow-hidden
                    ${isSub ? 'bg-stripes-zinc' : 'bg-[#15191f]'}`}>
      
      {/* SUBSTITUTE STRIPE PATTERN OVERLAY */}
      {isSub && <div className="absolute inset-0 opacity-5 pointer-events-none" 
      ...
      <div className="absolute inset-0 flex items-center justify-between px-4 z-10 transition-transform duration-300 group-hover:-translate-y-full">
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
             {avatarUrl ? (
               <img src={avatarUrl} alt="" className="w-8 h-8 rounded-md object-cover border border-zinc-700" />
             ) : (
               <div className="w-8 h-8 rounded-md bg-zinc-800 flex items-center justify-center border border-zinc-700 text-zinc-500">
          ...
          <div className="flex flex-col justify-center">
            <div className="flex items-center gap-2">
                ...
                {isSub && <span className="text-[9px] bg-yellow-900/30 text-yellow-500 px-1.5 py-0.5 rounded font-mono font-bold tracking-wider">SUB</span>}
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col items-end">
          {typeof elo === 'number' ? (
            <span className="text-xs font-mono font-bold text-[#ff5500]">
                {elo} <span className="text-zinc-600 text-[9px]">ELO</span>
            </span>
          ) : (
      ...
      {/* Hover Actions (Slides Up from Bottom for Tactical Feel) */}
      <div className="absolute inset-0 z-20 flex items-center justify-between px-4 bg-[#15191f]
...
const TeamCard = ({ team }) => {
  ...
  return (
    <div className="group relative bg-[#0b0c0f] border border-zinc-800 hover:border-zinc-600 transition-all duration-300 flex flex-col h-full overflow-hidden"
         style={{ clipPath: 'polygon(0 0, 100% 0, 100% 90%, 92% 100%, 0 100%)' }}>
      
      {/* Decorative Corner */}
      <div className="absolute top-0 right-0 w-8 h-8 bg-zinc-800/20 -skew-x-12 translate-x-4 -translate-y-4"></div>

      {/* Team Header */}
      <div className="p-4 bg-gradient-to-r from-[#15191f] to-[#0b0c0f] border-b border-zinc-800 flex items-center justify-between relative overflow-hidden">
        <div className="flex items-center gap-3 z-10">
            {team.logo_url ? (
                <img src={team.logo_url} alt="" className="w-10 h-10 object-contain drop-shadow-md" />
            ) : (
            ...
            <div className="flex flex-col">
                <h3 className="text-zinc-100 font-black text-base truncate max-w-[140px]">{team.name}</h3>
                <span className="text-[10px] text-zinc-500 font-mono">SEED #{team.seed_number}</span>
            </div>
        </div>

        {/* TEAM FLAG (Absolute Right) */}
        {flagUrl && (
            <div className="relative group/flag">
                <img src={flagUrl} alt={team.region} className="w-8 h-5 rounded shadow-sm opacity-80 group-hover/flag:opacity-100 transition-opacity" />
                <div className="absolute right-0 -bottom-6 bg-black text-white text-[9px] px-2 py-1 rounded opacity-0 group-hover/flag:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-20">
                    {team.region}
                </div>
            </div>
        )}
      ...
      {team.discord_channel_url && (
        <a 
          ...
          target="_blank"
          rel="noreferrer"
          className="flex items-center justify-center gap-2 w-full py-3 mt-auto
                     bg-[#15191f] hover:bg-[#5865F2]/10 text-zinc-500 hover:text-[#5865F2]
                     transition-colors duration-200 text-[10px] font-bold uppercase tracking-widest border-t border-zinc-800"
        >
  ...
  if (loading) {
    return (
        <div className="flex flex-col items-center justify-center h-64 text-zinc-500 gap-4">
            ...
            <p className="text-xs font-bold tracking-widest uppercase">Syncing Roster...</p>
        </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 p-4 md:p-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-4 border-b border-zinc-800 pb-6">
            <div>
                <h2 className="text-3xl font-brand font-black text-white tracking-wide italic">
                    ROSTER <span className="text-[#ff5500]">INTEL</span>
                </h2>
                <p className="text-zinc-500 text-xs font-mono mt-1 uppercase tracking-widest">
                    Authorized Personnel Only • {teams.length} Squads Active
                </p>
            </div>
            <div className="w-full md:w-auto relative">
                ...
                <input 
                    ...
                    className="bg-[#0b0c0f] border border-zinc-800 text-white pl-10 pr-4 py-2 rounded w-full md:w-64 focus:border-[#ff5500] outline-none text-xs font-mono uppercase transition-colors"
                    value={searchTerm}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredTeams.length > 0 ? (
                ...
                <div className="col-span-full py-20 text-center text-zinc-500 flex flex-col items-center gap-3">
                    <AlertTriangle className="w-8 h-8 opacity-20 text-[#ff5500]" />
                    <span className="font-mono text-xs uppercase tracking-widest">No matching units found</span>
                </div>
            )}
        </div>
    </div>
