...
import { Check, AlertCircle, Clock, Shield, History, Zap } from 'lucide-react';
...
const VetoPanel = ({ match }) => {
  ...
  const [timeLeft, setTimeLeft] = useState(60); // 60s standard
  ...
  // 3. TIMER LOGIC (Frontend Only for now)
  useEffect(() => {
    if (currentStep.action === "COMPLETE") return;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
          ...
          {MAP_POOL.filter(m => m.active).map(map => {
            ...
            return (
              <button
                ...
                `}
              >
                <img src={map.image} className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000" />
      ...
      {/* SIDE SELECTION UI (Hidden until needed) */}
