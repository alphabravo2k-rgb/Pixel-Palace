...
const NavButton = memo(({ view, currentView, onClick, label, icon: Icon }) => (
  <button 
    ...
    className={`relative px-6 py-3 flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-200
      ${currentView === view ? 'text-[#ff5500] bg-[#ff5500]/10 border-b-2 border-[#ff5500]' : 'text-zinc-500 hover:text-zinc-300 border-b-2 border-transparent'}`}
  >
...
const BracketView = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen">
        ...
        <LayoutContainer>
          <div className="flex justify-between items-end h-16">
            <div className="flex items-center gap-4 mb-4">
              <div className="w-10 h-10 bg-[#ff5500] flex items-center justify-center">
                <Trophy className="w-5 h-5 text-black fill-black" />
              </div>
              <div className="flex flex-col">
                <h1 className="text-xl font-black text-white italic tracking-tighter uppercase leading-none">PIXEL <span className="text-zinc-600">PALACE</span></h1>
                <span className="text-[9px] font-mono text-[#ff5500] uppercase tracking-[0.3em]">Operations Command</span>
              </div>
            </div>
      ...
      <footer className="border-t border-zinc-900 bg-[#060709] py-8 mt-auto text-center">
         <p className="text-[10px] text-zinc-700 font-mono uppercase tracking-widest">Pixel Palace {'//'} {APP_VERSION}</p>
      </footer>
      {(session.isAuthenticated || permissions.isSpectator) && <MatchModal match={selectedMatch} onClose={() => setSelectedMatch(null)} />}
    </div>
  );
};
